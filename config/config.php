...
$config['view']['name']             = 'Мы — Antilamo';                                      // название сайта
$config['view']['description']      = 'Antilamo — некоммерческая общественная организация'; // seo description
$config['view']['keywords']         = 'antilamo, жизнь, позитив';                           // seo keywords
 ...
 */
$config['path']['root']['web']        = 'http://we.antilamo.ru';          // полный WEB адрес сайта
$config['path']['root']['server']     = '/var/www/we.antilamo.ru';        // полный путь до сайта в файловой системе
/**
...
$config['sys']['mail']['from_email']       = 'info@antilamo.ru';     // мыло с которого отправляются все уведомления
$config['sys']['mail']['from_name']        = 'Почтовый голубь Antilamo';  // имя с которого отправляются все уведомления
...
$config['sys']['mail']['smtp']['user']     = 'info@antilamo.ru';     // настройки SMTP - пользователь
$config['sys']['mail']['smtp']['password'] = 'zw987412365bcd';       // настройки SMTP - пароль
 ...
 */
$config['acl']['create']['blog']['rating']                    =  15;  // порог рейтинга, при котором юзер может создать коллективный блог
$config['acl']['create']['comment']['rating']                 = -70;  // порог рейтинга, при котором юзер может добавлять комментарии
$config['acl']['create']['comment']['limit_time']             = 120;  // время в секундах между постингом комментариев, если 0, то ограничение по времени не будет работать
$config['acl']['create']['comment']['limit_time_rating']      =  -6;  // рейтинг, выше которого перестаёт действовать ограничение по времени на постинг комментов. Не имеет смысла при $config['acl']['create']['comment']['limit_time']=0
$config['acl']['create']['topic']['limit_time']               = 300;  // время в секундах между созданием записей, если 0, то ограничение по времени не будет работать
$config['acl']['create']['topic']['limit_time_rating']        =  10;  // рейтинг, выше которого перестаёт действовать ограничение по времени на создание записей
...
$config['acl']['create']['talk_comment']['limit_time']        =  30;  // время в секундах между отправкой инбоксов, если 0, то ограничение по времени не будет работать
...
$config['acl']['vote']['comment']['rating']                   = -10;  // порог рейтинга, при котором юзер может голосовать за комментарии
$config['acl']['vote']['blog']['rating']                      =   0;  // порог рейтинга, при котором юзер может голосовать за блог
$config['acl']['vote']['topic']['rating']                     =  -5;  // порог рейтинга, при котором юзер может голосовать за топик
$config['acl']['vote']['user']['rating']                      =   5;  // порог рейтинга, при котором юзер может голосовать за пользователя
$config['acl']['vote']['topic']['limit_time']                 = 60*60*24*30; // ограничение времени голосования за топик
$config['acl']['vote']['comment']['limit_time']               = 60*60*24*30; // ограничение времени голосования за комментарий
/**
 * Настройки модулей
 */
// Модуль Blog
$config['module']['blog']['per_page']          =  20;         // число блогов на страницу
$config['module']['blog']['personal_good']     = -10;         // рейтинг топика в персональном блоге, ниже которого он считается плохим
$config['module']['blog']['collective_good']   = -10;         // рейтинг топика в коллективных блогах, ниже которого он считается плохим
$config['module']['blog']['index_good']        =   8;         // рейтинг топика, выше которого (включительно) он попадает на главную
$config['module']['blog']['encrypt']           = 'weareantilamoiamcrazybecauseilove'; // Ключ XXTEA шифрования идентификаторов в ссылках приглашения в блоги
...
$config['module']['topic']['title_min_length'] = 5;           // минимальное количество символов в заголовке топика
$config['module']['topic']['title_max_length'] = 200;         // максимальное количество символов в заголовке топика
$config['module']['topic']['min_length']       = 100;         // минимальное количество символов в одном топике
...
// Модуль Comment
$config['module']['comment']['per_page']            = 10;          // число комментариев на одну страницу (это касается только полного списка комментариев прямого эфира)
$config['module']['comment']['bad']                 = -10;         // рейтинг комментария, начиная с которого он будет скрыт
$config['module']['comment']['max_tree']            = 7;           // максимальная вложенность комментов при отображении
$config['module']['comment']['min_length']          = 2;           // минимальное количество символов в одном комментарии
$config['module']['comment']['max_length']          = 3000;        // максимальное количество символов в одном комментарии
...
$config['module']['talk']['encrypt']    = 'antilamoforeverandwearethechampions'; // ключ XXTEA шифрования идентификаторов в ссылках
// Модуль Lang
$config['module']['lang']['delete_undefined'] = false;  // если установлена true, то модуль будет автоматически удалять из языковых конструкций переменные вида %%var%%, по которым не была произведена замена
// Модуль Notify
$config['module']['notify']['delayed']        = false;  // указывает на необходимость использовать режим отложенной рассылки сообщений на e-mail
$config['module']['notify']['insert_single']  = false;  // если опция установлена в true, система будет собирать записи заданий удаленной публикации, для вставки их в базу единым INSERT
...
$config['module']['image']['default']['watermark_text']       = '© Antilamo';
...
$config['db']['params']['pass']   = 'zw987412365bcd';
$config['db']['params']['type']   = 'mysql';
$config['db']['params']['dbname'] = 'we_antilamo_ru';
/**
 * Настройка таблиц базы данных
 */
$config['db']['table']['prefix'] = 'war_';
...
$config['block']['rule_tag'] = array(
	'action'  => array( 'tag' ),
	'blocks'  => array( 'right' => array('tags','stream','blogs') ),
);
